...
#软件安装
rpm -q bc &>/dev/null;[ $? -gt 0 ] && yum -y install bc &>/dev/null
rpm -q net-tools &>/dev/null;[ $? -gt 0 ] && yum -y install net-tools &>/dev/null
...
USERs=0
echo "$(echo -e "用户名 UID GID HOME SHELL 最后一次登录"
for shell in $(grep -v "/sbin/nologin" /etc/shells); do
    for username in $(grep "$shell" /etc/passwd | awk -F: '{print $1}'); do
        userLastLogin="$(getUserLastLogin "$username" | tr ' ' '_')"
        echo "$pwdfile" | grep -w "$username" | grep -w "$shell" | awk -F: -v lastlogin="$userLastLogin" '{print $1,$3,$4,$6,$7,lastlogin}'
    done
    USERs=$((USERs+$(echo "$pwdfile" | grep "$shell" | wc -l)))
done
)" | column -t
echo ""
...
function getSSHStatus(){
    ...
    Protocol_Version=$(rpm -qi openssh-server |grep Version |awk '{{ print $3}}')
...
function getNTPStatus(){
    ...
    if [ -e /etc/chrony.conf ];then
        ...
        echo ""
        echo -e "\033[36m*************Chronyd服务配置******************\033[0m"
        cat /etc/chrony.conf 2>/dev/null | grep -v "^#" | sed '/^$/d'
    fi
}
