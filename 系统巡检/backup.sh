...
function beifen() {
  ...
  function zidingyi() {
    read -p "请输入数据库用户名称" DB_USER
    read -p "请输入数据库密码" DB_PWD
  ...
  #执行mysql的备份数据库的指令
  mysqldump -u${DB_USER} -p${DB_PWD} --host=$HOST $DATABASE | gzip >$BACKUP/$DATETIME.sql.tgz
  #打包备份文件
  #删除临时目录

  #删除10天前的备份文件(-exec rm -rf {} \是固定写法，删除查询出来的数据)
  find $BACKUP -mtime +10 -name "*.sql.tgz" -exec rm -rf {} \;
  echo "==========备份完成==========="
}
