...
install_haproxy() {
    [ -d ${HAPROXY_INSTALL_DIR} ] && {
        ${COLOR}"Haproxy已存在，安装失败"${END}
        exit
    }
    ...
    if [ ${OS_ID} == "CentOS" -o ${OS_ID} == "Rocky" ] &>/dev/null; then
        ...
        apt update &>/dev/null
        apt -y install gcc make openssl libssl-dev libpcre3 libpcre3-dev zlib1g-dev libreadline-dev libsystemd-dev &>/dev/null
    fi
    tar xf ${LUA_FILE}
    ...
    [ $? -eq 0 ] && $COLOR"Haproxy编译安装成功"$END || {
        $COLOR"Haproxy编译安装失败,退出!"$END
        exit
    }
...
EOF
    ...
    systemctl is-active haproxy &>/dev/null && ${COLOR}"Haproxy 服务启动成功!"${END} || {
        ${COLOR}"Haproxy 启动失败,退出!"${END}
        exit
    }
