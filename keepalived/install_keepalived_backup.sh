...
check_file() {
    ...
    if [ ! -e ${KEEPALIVED_FILE} ]; then
        ...
        wget ${KEEPALIVED_URL}${KEEPALIVED_FILE} || {
            ${COLOR}"Keepalived源码包下载失败"${END}
            exit
        }
    else
        ${COLOR}"相关文件已准备好"${END}
...
EOF
    ...
    if [[ ${OS_RELEASE_VERSION} == 8 ]] &>/dev/null; then
        ...
        apt update &>/dev/null
        apt -y install make gcc ipvsadm build-essential pkg-config automake autoconf iptables-dev libipset-dev libnl-3-dev libnl-genl-3-dev libssl-dev libxtables-dev libip4tc-dev libip6tc-dev libipset-dev libmagic-dev libsnmp-dev libglib2.0-dev libpcre2-dev libnftnl-dev libmnl-dev libsystemd-dev &>/dev/null
    fi
    ...
    [ $? -eq 0 ] && ${COLOR}"Keepalived编译安装成功"${END} || {
        ${COLOR}"Keepalived编译安装失败,退出!"${END}
        exit
    }
...
EOF
    ...
    systemctl is-active keepalived &>/dev/null && ${COLOR}"Keepalived 服务启动成功!"${END} || {
        ${COLOR}"Keepalived 启动失败,退出!"${END}
        exit
    }
